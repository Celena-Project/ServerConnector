...
export class MyWebSocketServer {
    ...
    private connectionHandlers:  ((message: any) => void)[] = [];
    public onConnection(action: (message: any) => void): void{
        this.connectionHandlers.push(action);
    }
    private async onRequest(request) {
        ...
        connection.on('message', async msg => {
            ...
            if(message != ""){
                if(message != undefined && message.startsWith("auth=")){
                    if(this.authChecker != undefined){
                        ...
                        let key = message.split("auth=")[1].split("key=")[1];
                    ...
                    for(let k in this.connections){
                        if(this.connections[k].connection == connection)
                        {
                           ...
                           for(let k in this.connectionHandlers){
                               this.connectionHandlers[k].call(message)
                           }
                        }
                    }
                }else{
    ...
    private subscribers: [{key: string, action: any, autoResponse: boolean}]
    public subscribe(key: string, action: (message: MyRequestMessage) => void, autoResponse: boolean = false): void{
        if(this.subscribers == undefined) this.subscribers = [{key, action, autoResponse}]
        else this.subscribers.push({key, action, autoResponse});
    }
    ...
    private onMessage(connection: { connection, id: string }, message: IMyMessage){
        ...
        if(message.messageType == "req" || message.messageType == 0){
            ...
            if(filtered.length > 0){
                ...
                filtered.forEach(x => {
                    x.action(reqMessage)
                    if(x.autoResponse == true){
                        this._response(reqMessage, "auto-response=successfully")
                    }
                })
            }
